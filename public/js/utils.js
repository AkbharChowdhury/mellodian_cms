...
// const formatDate = (date) => moment(date1, 'YYYY-MM-DD').format('d MMMM YYYY');
const formatDate = (date)=> moment(new Date(date)).format("Do MMMM YYYY");

const adultSupervision = ()=> `<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-exclamation-triangle" viewBox="0 0 16 16">
...
const renderBuyButton = (customerID,url) => {
    if (customerID == 0) {
        return ' <button class="btn btn-success text-capitalize" disabled>buy ticket</button>';

    }
    if (customerID != 0) {
        return `  <a class="btn btn-success text-capitalize" href="${url}">buy ticket</a>`;

    }
}
export { formatMoney, setMessage, formatTime , formatDate, adultSupervision, renderBuyButton};
